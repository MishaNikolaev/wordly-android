...
class GetCardSessionUseCaseTest {
	...
	private val cardPracticeRepository: CardPracticeRepository = mock()
	private val getCardSessionUseCase = GetCardSessionUseCase(cardPracticeRepository)
	...
	fun `invoke EXPECT repository get card session`() = runTest {
		whenever(cardPracticeRepository.getCardSession("science")) doReturn words

		getCardSessionUseCase("science")

		verify(cardPracticeRepository).getCardSession("science")
	}
