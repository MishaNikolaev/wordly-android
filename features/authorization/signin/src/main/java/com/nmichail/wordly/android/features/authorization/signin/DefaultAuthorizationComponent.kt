...
import com.nmichail.wordly.android.features.authorization.signup.DefaultSignUpComponent
	...
	private fun child(
		...
		when (config) {
			AuthorizationConfig.SignIn -> AuthorizationComponent.Child.SignIn(
				DefaultSignInComponent(componentContext),
			)
			AuthorizationConfig.SignUp -> AuthorizationComponent.Child.SignUp(
				DefaultSignUpComponent(componentContext),
			)
		}
}
