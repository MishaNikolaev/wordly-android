...
internal class DefaultProfileComponent @AssistedInject constructor(
		...
		componentScope().launch {
			for (label in store.labelsChannel(lifecycle)) {
				when (label) {
					...
					ProfileStore.Label.OpenNetworkSelection -> onOpenNetworkSelection()
				}
			}
		}
	}
	...
	override fun handleOpenNetworkSelection() {
		store.accept(ProfileStore.Intent.OpenNetworkSelection)
	}
